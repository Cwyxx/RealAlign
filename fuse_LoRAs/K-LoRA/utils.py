...
def insert_sd_klora_to_unet(
    ...
    # print(f"loading lora weight from {lora_weights_content_path}")
    lora_weights_content = get_lora_weights(lora_weights_content_path)
    # print(f"loading lora weight from {lora_weights_style_path}")
    ...
    # You will need to define this function, which will become the NEW forward pass
    # for your linear layers.
    def lora_forward(self, x):
        # self here is the original Linear layer object (e.g., attn_module.to_q)
        
        # First, perform the original linear layer's operation
        original_output = self.lora_original_forward(x)
        
        # Then, if a lora_layer has been attached, apply it and add the result
    ...
    # --- THE FIX IS APPLIED BELOW FOR EACH LAYER ---
    
        # 1. Modify the Query layer (to_q)
        lora_layer_q = initialize_klora_layer(
            **kwargs,
            part="to_q",
            in_features=attn_module.to_q.in_features,
            out_features=attn_module.to_q.out_features,
        )
        # Step 1: Attach the LoRA layer as a new attribute
        attn_module.to_q.lora_layer = lora_layer_q
        # Step 2: Hijack the forward pass
        attn_module.to_q.lora_original_forward = attn_module.to_q.forward
        attn_module.to_q.forward = MethodType(lora_forward, attn_module.to_q)

        # 2. Modify the Key layer (to_k)
        lora_layer_k = initialize_klora_layer(
            **kwargs,
            part="to_k",
            in_features=attn_module.to_k.in_features,
            out_features=attn_module.to_k.out_features,
        )
        attn_module.to_k.lora_layer = lora_layer_k
        attn_module.to_k.lora_original_forward = attn_module.to_k.forward
        attn_module.to_k.forward = MethodType(lora_forward, attn_module.to_k)

        # 3. Modify the Value layer (to_v)
        lora_layer_v = initialize_klora_layer(
            **kwargs,
            part="to_v",
            in_features=attn_module.to_v.in_features,
            out_features=attn_module.to_v.out_features,
        )
        attn_module.to_v.lora_layer = lora_layer_v
        attn_module.to_v.lora_original_forward = attn_module.to_v.forward
        attn_module.to_v.forward = MethodType(lora_forward, attn_module.to_v)

        # 4. Modify the Output layer (to_out[0])
        lora_layer_out = initialize_klora_layer(
            **kwargs,
            part="to_out.0",
            in_features=attn_module.to_out[0].in_features,
            out_features=attn_module.to_out[0].out_features,
        )
        attn_module.to_out[0].lora_layer = lora_layer_out
        attn_module.to_out[0].lora_original_forward = attn_module.to_out[0].forward
        attn_module.to_out[0].forward = MethodType(lora_forward, attn_module.to_out[0])
