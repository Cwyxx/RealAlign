...
training_sd35m also accepts a fresh env created from its own `setup.py` (`pip install -e .`), pinned to `torch==2.6.0`, `diffusers==0.33.1`, `transformers==4.40.0`, `accelerate==1.4.0`, Python 3.10.
...
| `training_sd35m/` | `accelerate launch --config_file scripts/accelerate_configs/multi_gpu.yaml scripts/train_*.py --config config/grpo.py:VARIANT` | same `:variant` selector pattern |

The `:VARIANT` config selector (training_sd35m) is a custom convention where the name after the colon picks a top-level function in the config module (e.g. `sd3_geneval`, `general_ocr_wan2_1`, `pickscore_sd3_fast_nocfg`). Look in the config file to enumerate valid variants before guessing.
...
- **`training_sd35m/diffusers_patch/`** overrides pieces of `diffusers` (notably `pipeline_with_logprob.py`, SD3/Flux SDE samplers, `train_dreambooth_lora_sd3.py`). When debugging sampling/log-prob issues, inspect the patch before assuming upstream `diffusers` behavior.
- **Multi-reward training (Flow-GRPO)** takes a dict like `{"pickscore": 0.5, "ocr": 0.2, "aesthetic": 0.3}`. Supported reward names are defined in `training_sd35m/flow_grpo/rewards.py`; some (GenEval) require running a separate reward server and may need their own conda env. DeQA / UnifiedReward / HPSv3 are loaded locally and used by the evaluation harness in `training_sd35m/evaluation/`.
...
- For RealAlign's two-stage training there is exactly one canonical trainer per (stage × model): `training_sd15/stage{1_diffusion_dro,2_dpo}/train-*.py` for SD-1.5 and `training_sd35m/scripts/train-sd-3-5-medium-{irl,dpo}.py` for SD-3.5-M. Historical ablation variants (`-w_sft`, `-curriculum_learning`, `-l_diff_coeffi`, `_maskdpo`, `-dpo_next`, `-w_sft_dgr`, `-only_sft`, `-iterative_dpo_sft`, `-dpo-sft`) have been removed; do not reintroduce sibling scripts. Edit the relevant `*.sh` launcher's hyperparameters in place rather than forking the trainer.
- The `notebook/` folder now contains paper-facing analysis artifacts. Keep filenames stable unless the corresponding paper figure/reference is updated.
