...
- **`benchmark-evaluation/`** — DPG-Bench evaluation scripts for SD-1.5 and SD-3.5-M.
...
- **`.gitignore`** excludes vendored evaluation deps (`evaluate_metric/hf_cache`, `evaluate_metric/t2v_metrics_github`, `evaluate_metric/MA-AGIQA`, `evaluate_metric/PKU-AIGIQA-4K`, `evaluate_metric/General-Visual-Quality-RL`) — these directories may be referenced by scripts but are not checked in.
...
- The `notebook/` folder contains throwaway helpers — do not refactor it unless asked.
