...
def main(args):
    ...
    if args.lora_hf_path is not None and args.lora_hf_path:
    ...
    elif len(args.checkpoint_paths) > 0:
        pipeline.transformer = get_peft_model(pipeline.transformer, transformer_lora_config)
        for lora_idx, lora_path in enumerate(args.checkpoint_paths):
            ...
            print(f"Loading LoRA {args.adapter_names[lora_idx]}/{args.lora_weights[lora_idx]}, weights from: {lora_path}")
            pipeline.transformer.load_adapter(lora_path, adapter_name=args.adapter_names[lora_idx], is_trainable=False)
            
        if args.combination_type in ["cat", "linear"]:
            pipeline.transformer.add_weighted_adapter(
        ...
        pipeline.transformer.set_adapter("GGBond")
        print(f"Activate Lora: {pipeline.transformer.active_adapters}")
