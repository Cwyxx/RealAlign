...
class TextPromptDataset(Dataset):
    def __init__(self, dataset, split="train"):
        ...
        if split == "test":
            self.prompts = self.prompts[0:48]
...
def main(_):
    ...
    if is_main_process(rank):
        ...
        swanlab.init(project="flow-grpo", name=os.path.basename(config.run_name), config=config.to_dict(), dir=log_dir)
    logger.info(f"\n{config}")
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        while current_group_num < config.num_groups:
            ...
            prompt_embeds, pooled_prompt_embeds = compute_text_embeddings(prompts, text_encoders, tokenizers, max_sequence_length=128, device=device)
            prompt_ids = tokenizers[0](prompts, padding="max_length", max_length=256, truncation=True, return_tensors="pt").input_ids.to(device)
            ...
            if (single_prompt_gathered_avg_rewards < config.negative_threshold).all():
                ...
                if log_images_to_swanlab and is_main_process(rank):
                    ...
                    log_images_to_swanlab = False
                current_group_num += 1
                current_group_bar.set_description(f"Consume Prompts: {consume_prompt_num}")  
