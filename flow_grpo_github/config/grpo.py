...
def dinov2_sd3_4gpu():
    ...
    config.run_name = "dinov2-default_config"
    ...
    config.train.gradient_accumulation_steps = (config.sample.train_batch_size // config.train.batch_size) * config.sample.num_batches_per_epoch // 2 # config.sample.num_batches_per_epoch//2
    config.train.num_inner_epochs = 1
    config.train.timestep_fraction = 0.99
    config.train.beta = 0.01
    config.sample.global_std = True
    ...
    config.eval_freq = 10
    config.save_dir = f'/data_center/data2/dataset/chenwy/21164-data/flow-grpo/model-ckpt/{config.run_name}'
    config.reward_fn = {
        "dinov2": 1.0,
    }
