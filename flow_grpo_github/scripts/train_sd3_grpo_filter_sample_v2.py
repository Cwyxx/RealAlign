...
def main(_):
    ...
    while True:
        ...
        prompts = []
        current_group_num = 0
        current_group_bar = tqdm(total=config.num_groups, desc="Processing Groups", disable=not accelerator.is_local_main_process, dynamic_ncols=True, position=0)
        
        while current_group_num < config.num_groups:
            train_sampler.set_epoch(consume_prompt_num) # train_sampler.set_epoch(epoch * config.sample.num_batches_per_epoch + i)
            ...
            single_prompt_gathered_avg_rewards = accelerator.gather(rewards["avg"]).cpu().numpy()
