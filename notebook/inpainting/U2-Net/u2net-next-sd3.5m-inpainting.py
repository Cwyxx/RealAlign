...
def main(args):
    ...
    pipeline = StableDiffusion3InpaintPipeline.from_pretrained("stabilityai/stable-diffusion-3.5-medium", torch_dtype=torch.float16)
    pipeline = pipeline.to(device)
    # pipeline.transformer.to(device, dtype=torch.float16)
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing images"):
        ...
        try:
            ...
            fake_image = pipeline(prompt=prompt, image=real_image, mask_image=saliency_mask, strength=1.0).images[0]
