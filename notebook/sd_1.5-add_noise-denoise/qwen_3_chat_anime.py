    ...
    messages = [
        {
            ...
            "content": [
                ...
                {"type": "text", "text": "You are an image classification assistant. Your task is to determine if the given image is an **artwork or anime style** image. Respond with **only** 'yes' if it is an artwork or anime style, and 'no' if it is not. Do not provide any other text or explanation."},
            ],
        }
    ]
...
if __name__ == "__main__":
    ...
    if uid_file is not None:
        ...
        for f in os.listdir(image_dir):
            file_path = image_dir_path / f
            if file_path.is_file() and any(f.lower().endswith(ext.lower()) for ext in ext_list):
                all_uids.append(os.path.splitext(f)[0])
    ...
    for idx, uid in enumerate(uids_to_process, 1):
        ...
        if image_path is None:
            ...
            result = {"uid": uid, "image_path": "", "anime": "", "error": "Image file not found"}
        ...
        # Generate caption
        try:
            anime = generate_anime(image_path)
            print(f"  Anime: {anime}")
            result = {"uid": uid, "image_path": image_path, "anime": anime, "error": ""}
            results.append(result)
            # Save results immediately after getting a caption
            save_results(results, output_file)
        except Exception as e:
            print(f"  Error processing {uid}: {e}")
            result = {"uid": uid, "image_path": image_path, "anime": "", "error": str(e)}
            results.append(result)
    ...
    print(f"Successfully processed (this run): {sum(1 for r in results[len(existing_results):] if r.get('anime', '').strip())}")
    print(f"Failed/Not found (this run): {sum(1 for r in results[len(existing_results):] if not r.get('anime', '').strip())}")
    print(f"Total results: {len(results)}")
    print(f"Total with prompts: {sum(1 for r in results if r.get('anime', '').strip())}")
