...
def prepare_extra_step_kwargs(pipeline, generator, eta):
    ...
    accepts_generator = "generator" in set(inspect.signature(pipeline.scheduler.step).parameters.keys())
...
class RealFakePairGenerator:
    def __init__(self, args):
        ...
        self.noise_scheduler.set_timesteps(self.total_inference_step, device=self.device) # use 50 timesteps to generate image from a clean noise.
        ...
        self.add_noise_step = None # random add noise step
        self.denoise_step = None # random denoise step
    ...
    def denoise(self, noisy_latent, prompt=None):
        ...
        # Calculate the starting index for denoising
        start_idx = self.total_inference_step - self.add_noise_step
        # Ensure start_idx is valid
        start_idx = max(0, min(start_idx, len(self.inference_timesteps) - 1))
        end_idx = start_idx + self.denoise_step
        # Ensure we don't exceed the available timesteps
        end_idx = min(end_idx, len(self.inference_timesteps))
        # Ensure we have at least one timestep
        if end_idx <= start_idx:
            end_idx = min(start_idx + 1, len(self.inference_timesteps))
        timesteps = self.inference_timesteps[start_idx:end_idx]
        print(f"start_idx: {start_idx}, end_idx: {end_idx}, len timesteps: {len(timesteps)}")
    ...
    def process_image(self, image_path, prompt=None):
        ...
        add_noise_step = random.randint(10, 15)
        self.add_noise_step = add_noise_step
        self.denoise_step = add_noise_step
        
        # Ensure add_noise_step doesn't exceed available timesteps
        max_add_noise_step = self.total_inference_step - 1
        if self.add_noise_step > max_add_noise_step:
            self.add_noise_step = max_add_noise_step
            print(f"Warning: add_noise_step adjusted to {self.add_noise_step}")
        
        # Calculate noise timestep index
        noise_timestep_idx = self.total_inference_step - self.add_noise_step
        
        if noise_timestep_idx >= len(self.inference_timesteps):
            noise_timestep_idx = len(self.inference_timesteps) - 1
            self.add_noise_step = self.total_inference_step - noise_timestep_idx
            print(f"Warning: noise_timestep_idx adjusted, add_noise_step is now {self.add_noise_step}")
        
        # Add noise
        noise_timestep = self.inference_timesteps[noise_timestep_idx]
        noisy_latent = self.add_noise(latent, noise_steps=noise_timestep)
