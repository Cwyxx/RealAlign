...
class StableDiffusion3Pipeline(diffusers.StableDiffusion3Pipeline):
    _callback_tensor_inputs = ["latents", "prompt_embeds", "pooled_prompt_embeds", "noise_pred", "sigma"]
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, (t, sigma) in enumerate(zip(timesteps, sigmas)):
                # print(f"timestep: {t}, sigma: {sigma}")
                if self.interrupt:
                    continue
...
class ReplayBuffer(PipelineCallback):
    ...
    def commit(self):
        ...
        for key in self.staging.keys():
            if key not in [self.time_key, "sigma"]:
        ...
        self.staging["sigma"] = torch.stack(self.staging["sigma"])
        ...
        if len(self.staging[self.time_key].shape) == 1:
            ...
            self.staging["sigma"] = self.staging["sigma"].unsqueeze(0).expand(B, T)
        else:
            ...
            self.staging["sigma"] = self.staging["sigma"].t()
    ...
    def callback_fn(
        ...
        self.push("sigma", callback_kwargs["sigma"], is_time_dependent=True)
    ...
    def tensor_inputs(self) -> List[str]:
        ...
        return ["latents", "noise_pred", "sigma"]
