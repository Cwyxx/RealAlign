...
def main(opt):
    ...
    for lora_dir, lora_weight, adapter_name in zip(opt.lora_dirs, opt.lora_weights, opt.adapter_names):
        model.load_lora_weights(lora_dir, weight_name="pytorch_lora_weights.safetensors", adapter_name=adapter_name)
        print(f"Loaded LoRA from {lora_dir} with weight {lora_weight} for adapter {adapter_name}")
    
    if len(opt.lora_dirs) > 1:
        model.set_adapters(opt.adapter_names, adapter_weights=opt.lora_weights)
        print(f"Set adapters {opt.adapter_names} with weights {opt.lora_weights}")
