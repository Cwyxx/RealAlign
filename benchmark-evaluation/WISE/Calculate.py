...
def process_jsonl_file_segment(file_path, category_arg=None):
    ...
    segment_consistency = defaultdict(list)
    segment_realism = defaultdict(list)
    segment_aesthetic_quality = defaultdict(list)
            ...
            for line_num, line in enumerate(file, 1):
                try:
                    ...
                    if 1 <= prompt_id <= 400:
                        ...
                        cat = None
                    if cat:
                        segment_scores[cat].append(wiscore)
                        segment_consistency[cat].append(consistency)
                        segment_realism[cat].append(realism)
                        segment_aesthetic_quality[cat].append(aesthetic_quality)
                    else:
    ...
    return {
        ...
        'consistency': segment_consistency,
        'realism': segment_realism,
        'aesthetic_quality': segment_aesthetic_quality,
...
def main():
    ...
    aggregated_consistency = defaultdict(list)
    aggregated_realism = defaultdict(list)
    aggregated_aesthetic_quality = defaultdict(list)
    ...
    for file_data in all_raw_results:
        ...
        current_file_avg_consistency = {}
        current_file_avg_realism = {}
        current_file_avg_aesthetic_quality = {}
        ...
        for category, scores_list in file_data['scores'].items():
            ...
            aggregated_consistency[category].extend(file_data['consistency'][category])
            aggregated_realism[category].extend(file_data['realism'][category])
            aggregated_aesthetic_quality[category].extend(file_data['aesthetic_quality'][category])
            ...
            if scores_list: # Only add to individual file report if samples exist
                ...
                current_file_avg_consistency[category] = sum(file_data['consistency'][category]) / len(file_data['consistency'][category])
                current_file_avg_realism[category] = sum(file_data['realism'][category]) / len(file_data['realism'][category])
                current_file_avg_aesthetic_quality[category] = sum(file_data['aesthetic_quality'][category]) / len(file_data['aesthetic_quality'][category])
        ...
        final_file_reports[file_path] = {
            ...
            'avg_consistency': current_file_avg_consistency,
            'avg_realism': current_file_avg_realism,
            'avg_aesthetic_quality': current_file_avg_aesthetic_quality,
    ...
    for file_path, file_data in final_file_reports.items():
        ...
        if not categories_to_print:
            ...
            for category in categories_to_print:
                ...
                avg_cons = file_data['avg_consistency'].get(category, 0)
                avg_real = file_data['avg_realism'].get(category, 0)
                avg_aes = file_data['avg_aesthetic_quality'].get(category, 0)
                sample_count = file_data['num_processed_samples'].get(category, 0)
                print(f"  Category: {category}")
                print(f"    Average WiScore: {avg_score:.3f}")
                print(f"    Average Consistency: {avg_cons:.3f}  Realism: {avg_real:.3f}  Aesthetic Quality: {avg_aes:.3f}")
    ...
    overall_avg_consistency = {
        cat: sum(aggregated_consistency[cat]) / len(aggregated_consistency[cat]) if aggregated_consistency[cat] else 0
        for cat in aggregated_scores
    }
    overall_avg_realism = {
        cat: sum(aggregated_realism[cat]) / len(aggregated_realism[cat]) if aggregated_realism[cat] else 0
        for cat in aggregated_scores
    }
    overall_avg_aesthetic_quality = {
        cat: sum(aggregated_aesthetic_quality[cat]) / len(aggregated_aesthetic_quality[cat]) if aggregated_aesthetic_quality[cat] else 0
        for cat in aggregated_scores
    }
    overall_num_samples = {
        category: len(scores)
    ...
    if not overall_categories_to_print and args.category != 'all':
        ...
        for category in overall_categories_to_print:
            print(f"  Category: {category}")
            print(f"    Average WiScore: {overall_avg_scores.get(category, 0):.3f}")
            print(f"    Average Consistency: {overall_avg_consistency.get(category, 0):.3f}  Realism: {overall_avg_realism.get(category, 0):.3f}  Aesthetic Quality: {overall_avg_aesthetic_quality.get(category, 0):.3f}")
    ...
    if args.category == 'all' and all_categories_have_overall_samples:
        ...
        def _overall_mean(agg_dict):
            all_vals = []
            for cat in ordered_categories:
                all_vals.extend(agg_dict.get(cat, []))
            return sum(all_vals) / len(all_vals) if all_vals else 0
        overall_cons_mean = _overall_mean(aggregated_consistency)
        overall_real_mean = _overall_mean(aggregated_realism)
        overall_aes_mean = _overall_mean(aggregated_aesthetic_quality)
        
        print("\n--- Overall WiScore Across All Categories ---")
        print(f"Overall WiScore: {overall_wiscore:.3f}")
        print("Cultural\tTime\tSpace\tBiology\tPhysics\tChemistry\tOverall")
        print(f"{cultural_score:.3f}\t\t{time_score:.3f}\t{space_score:.3f}\t{biology_score:.3f}\t{physics_score:.3f}\t{chemistry_score:.3f}\t\t{overall_wiscore:.3f}")
        print("\n--- Consistency / Realism / Aesthetic Quality (per category & overall) ---")
        print("Consistency:\t" + "\t".join(f"{overall_avg_consistency.get(c, 0):.3f}" for c in ordered_categories) + f"\t{overall_cons_mean:.3f}")
        print("Realism:\t" + "\t".join(f"{overall_avg_realism.get(c, 0):.3f}" for c in ordered_categories) + f"\t{overall_real_mean:.3f}")
        print("Aesthetic:\t" + "\t".join(f"{overall_avg_aesthetic_quality.get(c, 0):.3f}" for c in ordered_categories) + f"\t{overall_aes_mean:.3f}")
